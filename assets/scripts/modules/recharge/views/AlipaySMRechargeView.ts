...
export default class AlipayRechargeView extends JPView {
    ...
    private alipaySMSettingInfo: RechargeSetting;
    initUsualCoin() {
        this.alipaySMSettingInfo = this.rechargeModel.getSettingInfoByType(PayType.ALIPAY_SM);
        if(!this.alipaySMSettingInfo) {
        ...
        if(this.alipaySMSettingInfo) {
            this.btns.forEach((btn, idx) => {
                cc.find('label', btn.node).getComponent(cc.Label).string = this.alipaySMSettingInfo.fastAmountList[idx] / 100 + '';
            });
        }

        if(this.alipaySMSettingInfo && this.alipaySMSettingInfo.minAmount && this.alipaySMSettingInfo.maxAmount) {
            this.inputTxt.placeholder = `建议充值${this.alipaySMSettingInfo.minAmount / 100}-${this.alipaySMSettingInfo.maxAmount / 100}元`;
        }
    }
    ...
    private async rechargeHandle() {
        ...
        if(!this.alipaySMSettingInfo.enable) {
        ...
        if(this.alipaySMSettingInfo.minAmount && this.alipaySMSettingInfo.minAmount / 100 > Number(this.inputTxt.string)) {
            this.pushTipsQueueSignal.dispatch(`充值金额必须在${this.alipaySMSettingInfo.minAmount / 100}-${this.alipaySMSettingInfo.maxAmount / 100}之间`);
            return;
        }

        if(this.alipaySMSettingInfo.maxAmount && this.alipaySMSettingInfo.maxAmount / 100 < Number(this.inputTxt.string)) {
            this.pushTipsQueueSignal.dispatch(`充值金额必须在${this.alipaySMSettingInfo.minAmount / 100}-${this.alipaySMSettingInfo.maxAmount / 100}之间`);
            return;
        }

        BaseWaitingPanel.show("正在登录");
        let task = this.rechargeServer.recharge(Number(this.inputTxt.string) * 100, PayType.ALIPAY_SM);
