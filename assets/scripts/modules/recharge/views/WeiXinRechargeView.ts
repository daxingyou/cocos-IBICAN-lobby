...
@ccclass
export default class WeiXinRechargeView extends JPView {
    ...
    private weixinSettingInfo: RechargeSetting;
    initUsualCoin() {
        this.weixinSettingInfo = this.rechargeModel.getSettingInfoByType(PayType.WEIXIN_H5);
        if(!this.weixinSettingInfo) {
        ...
        if(this.weixinSettingInfo) {
            this.btns.forEach((btn, idx) => {
                cc.find('label', btn.node).getComponent(cc.Label).string = this.weixinSettingInfo.fastAmountList[idx] / 100 + '';
            });
        }

        if(this.weixinSettingInfo && this.weixinSettingInfo.minAmount && this.weixinSettingInfo.maxAmount) {
            this.inputTxt.placeholder = `建议充值${this.weixinSettingInfo.minAmount / 100}-${this.weixinSettingInfo.maxAmount / 100}元`;
        }
    }
    ...
    private async rechargeHandle() {
        ...
        if(!this.weixinSettingInfo.enable) {
        ...
        if(this.weixinSettingInfo.minAmount && this.weixinSettingInfo.minAmount / 100 > Number(this.inputTxt.string)) {
            this.pushTipsQueueSignal.dispatch(`充值金额必须在${this.weixinSettingInfo.minAmount / 100}-${this.weixinSettingInfo.maxAmount / 100}之间`);
            return;
        }

        if(this.weixinSettingInfo.maxAmount && this.weixinSettingInfo.maxAmount / 100 < Number(this.inputTxt.string)) {
            this.pushTipsQueueSignal.dispatch(`充值金额必须在${this.weixinSettingInfo.minAmount / 100}-${this.weixinSettingInfo.maxAmount / 100}之间`);
            return;
        }

        BaseWaitingPanel.show("正在登录");
        let task = this.rechargeServer.recharge(Number(this.inputTxt.string) * 100, PayType.WEIXIN_H5);
