declare module jsb {
    ...
    export class AssetsManager {
        constructor(manifestUrl: string, storagePath: string, versionCompareHandle: (versionA: string, versionB: string) => number = null);

        setVersionCompareHandle(VersionCompareHandle: (versionA: string, versionB: string) => number): void;
        setVerifyCallback(VerifyCallback: (path: string, asset: { md5: string, size: number, path: string, compressed: boolean }) => boolean): void;
        setEventCallback(callback:(event: EventAssetsManager) => any);
